...
void ConfigUtils::initUnsetPropertiesWithDefaults(Config& config)
{
    ...
    for (uint16_t i = 0; i < HETRIGGER_COUNT; i++) {
        ...
        INIT_UNSET_PROPERTY(trigger, actuationPoint, HETRIGGER_DEFAULT_ACTUATION);
        INIT_UNSET_PROPERTY(trigger, rtPressSensitivity, HETRIGGER_DEFAULT_RT_PRESS);
        INIT_UNSET_PROPERTY(trigger, rtReleaseSensitivity, HETRIGGER_DEFAULT_RT_RELEASE);
        INIT_UNSET_PROPERTY(trigger, continuousRapidTrigger, HETRIGGER_DEFAULT_RT_CONTINUOUS);
        INIT_UNSET_PROPERTY(trigger, travelDeadzone, HETRIGGER_DEFAULT_DEADZONE);
    }

    // Seed each HE binding profile from the base bindings, so enabling a profile
    // in the web UI yields an editable copy rather than an empty grid.
    for (uint16_t p = 0; p < HE_PROFILE_COUNT - 1; p++) {
        HETriggerProfile& profile = config.addonOptions.heTriggerOptions.profileSets[p];
        if (profile.actions_count == 0) {
            for (uint16_t i = 0; i < HETRIGGER_COUNT; i++) {
                profile.actions[i] = config.addonOptions.heTriggerOptions.triggers[i].action;
            }
    ...
    config.addonOptions.heTriggerOptions.profileSets_count = HE_PROFILE_COUNT - 1;
